...
interface CategoryFilesProps {
  categoryId?: string; // This would come from URL params in real implementation
}

const CategoryFiles: React.FC<CategoryFilesProps> = ({ categoryId = "1" }) => {
  ...
  // Mock categories data (this would come from props or context in real app)
  ...
  // Mock files data filtered by category
  const allFiles: FileItem[] = [
    // Employee Manuals (categoryId: "1")
    {
      id: '1',
    ...
    // Forms & Applications (categoryId: "2")
    {
      id: '6',
      name: 'Leave Request Form.pdf',
      type: 'file',
      fileType: 'pdf',
      size: 345678,
    ...
    // Add more files for other categories as needed...
  ];

  // Filter files by current category
  const categoryFiles = allFiles.filter(file => file.categoryId === categoryId);

  const filteredAndSortedFiles = useMemo(() => {
    let filtered = categoryFiles.filter(file =>
    ...
    return filtered;
  }, [categoryFiles, searchQuery, sortBy, sortOrder]);
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className={`p-3 ${currentCategory.bgColor} rounded-lg`}>
              <CategoryIcon className={`w-8 h-8 ${currentCategory.color}`} />
            </div>
            <div>
          ...
          <nav className="flex items-center space-x-2 text-sm text-gray-600">
            <button onClick={() => window.location.href = '/'} className="hover:text-blue-600 transition-colors flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xl font-bold text-gray-900">{categoryFiles.length}</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xl font-bold text-gray-900">
                  {categoryFiles.filter(f => f.type === 'folder').length}
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xl font-bold text-gray-900">
                  {categoryFiles.filter(f => f.isStarred).length}
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xl font-bold text-gray-900">
                  {categoryFiles.filter(f => 
                    new Date().getTime() - f.modifiedAt.getTime() < 7 * 24 * 60 * 60 * 1000
                  ).length}
                </div>
              </div>
            </div>
        ...
        {filteredAndSortedFiles.length === 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">
              {searchQuery ? 'Try adjusting your search terms.' : 'Upload your first file to this category.'}
            </p>
